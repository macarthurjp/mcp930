document.addEventListener("DOMContentLoaded", function () {
  ...
  function bindEventsCarousel() {
    ...
    if (eventsLeft.dataset.bound !== "true") {
      ...
      eventsLeft.style.pointerEvents = "auto";
      eventsLeft.style.cursor = "pointer";
    ...
    if (eventsRight.dataset.bound !== "true") {
      ...
      eventsRight.style.pointerEvents = "auto";
      eventsRight.style.cursor = "pointer";
...
function getInviteesGap() {
  ...
  function bindInviteesCarousel() {
    ...
    if (inviteesLeft.dataset.bound !== "true") {
      inviteesLeft.addEventListener("click", () => {
        goToInviteesIndex(inviteesIndex - 1);
      });

      inviteesLeft.style.pointerEvents = "auto";
      inviteesLeft.style.cursor = "pointer";
      inviteesLeft.dataset.bound = "true";
    }

    if (inviteesRight.dataset.bound !== "true") {
      inviteesRight.addEventListener("click", () => {
        goToInviteesIndex(inviteesIndex + 1);
      });

      inviteesRight.style.pointerEvents = "auto";
      inviteesRight.style.cursor = "pointer";
  ...
  menuOverlay?.addEventListener("click", function () {
    ...
    /* -----------------------------------------
     GALERÍA SEMANAL - CARRUSEL SIN LIGHTBOX
  ----------------------------------------- */
  function bindWeeklyGalleryClicks() {
    document.querySelectorAll(
      ".weekly-card, .weekly-image-card, .weekly-gallery-card, .weekly-item, .semanal-card, .semanal-item"
  ...
  function getWeeklyGrid() {
    return document.getElementById("weeklyGalleryGrid");
  }

  function getWeeklyCards() {
    const grid = getWeeklyGrid();
    return grid ? Array.from(grid.querySelectorAll(".weekly-gallery-card")) : [];
  }

  function ensureWeeklyGalleryArrows() {
    document.querySelectorAll(
      '.weekly-gallery-arrow, [id="weeklyGalleryLeft"], [id="weeklyGalleryRight"]'
  ...
  function getWeeklyScrollStep() {
    const grid = getWeeklyGrid();
    const firstCard = getWeeklyCards()[0];
    if (!grid || !firstCard) return 0;
  ...
  function updateWeeklyGalleryArrows() {
    ensureWeeklyGalleryArrows();
  }

  function applyWeeklyGalleryCarouselLayout() {
    const grid = getWeeklyGrid();
    const cards = getWeeklyCards();
    if (!grid) return;

    grid.style.display = "flex";
    grid.style.flexWrap = "nowrap";
    grid.style.justifyContent = cards.length === 1 ? "center" : "flex-start";
    grid.style.alignItems = "stretch";
    grid.style.overflowX = cards.length > 1 ? "auto" : "hidden";
    grid.style.overflowY = "hidden";
    grid.style.scrollBehavior = "smooth";
    grid.style.scrollSnapType = cards.length > 1 ? "x mandatory" : "none";

    cards.forEach(function(card) {
      card.style.scrollSnapAlign = "center";
      card.style.flex = window.innerWidth <= 640 ? "0 0 88vw" : "0 0 400px";
      card.style.maxWidth = window.innerWidth <= 640 ? "88vw" : "400px";
    });
  }

  function bindWeeklyGalleryCarousel() {
    const grid = getWeeklyGrid();
    if (!grid) return;

    ensureWeeklyGalleryArrows();
    applyWeeklyGalleryCarouselLayout();

    if (grid.dataset.weeklyScrollBound !== "true") {
      grid.addEventListener("scroll", updateWeeklyGalleryArrows, { passive: true });
      grid.dataset.weeklyScrollBound = "true";
    }
...
`).join("");
  ...
  ----------------------------------------- */
    ...
    try {
      ...
      console.log("Datos semanal:", data.semanal || data.Semanal || []);
      ...
      const semanal = Array.isArray(data.semanal)
        ? data.semanal
        : Array.isArray(data.Semanal)
          ? data.Semanal
          : [];
      ...
      renderWeeklyGallery(semanal);
      bindWeeklyGalleryClicks();
      bindWeeklyGalleryCarousel();
...
function renderWeeklyGallery(items) {
  const grid = document.getElementById("weeklyGalleryGrid");
  if (!grid) return;

  const loadingText = document.getElementById("weeklyLoadingText");
  if (loadingText && loadingText !== grid) {
    loadingText.remove();
  }

  const weeklyItems = Array.isArray(items)
    ? items
        .filter(item => item && getField(item, ["fotoUrl", "FotoURL", "fotoURL", "url", "image", "imagen"]))
        .slice(0, 3)
    : [];

  grid.classList.remove("weekly-count-1", "weekly-count-2", "weekly-count-3");
  ...
  grid.classList.add("weekly-count-" + Math.min(weeklyItems.length, 3));

  grid.innerHTML = weeklyItems.map((item, index) => {
    const fotoUrl = getField(item, ["fotoUrl", "FotoURL", "fotoURL", "url", "image", "imagen"]);

    return `
      <article class="weekly-gallery-card">
        <img src="${escapeHtml(fotoUrl)}" alt="Imagen semanal ${index + 1}" loading="lazy" />
      </article>
    `;
  }).join("");
